...
# Tak, analiza klastrów pozwala zidentyfikować grupy dokumentów o podobnym słownictwie. 
# Jeśli w danym klastrze znajdują się dokumenty zawierające specyficzne terminy np.inflacja, PKB, 
# można wnioskować,że dotyczą one gospodarki.Klastrowanie ujawnia strukturę tematyczną 
# zbioru bez konieczności czytania każdego dokumentu z osobna.

# Pytanie 2:Jakich ogólnie obszarów tematycznych dotyczą klastry oraz ile jest tych obszarów?
# Zgodnie z kodem, analizowano podział na 2, 3 oraz 4 obszary tematyczne (klastry).
# Aby podać konkretne tematy, należy odczytać słowa z wygenerowanych chmur słów:
# - Jeśli k=3, mamy 3 obszary tematyczne.
# - Klaster 1: Ogólna retoryka państwowa i patriotyczna (America, country, will).
# - Klaster 2: Debata społeczno-polityczna dotycząca IMIGRACJI (immigration, american, country).
# - Klaster 3: ZARZĄDZANIE I ORGANIZACJA PRACY (management, project, system, work).
# Ostateczna liczba obszarów zależy od interpretacji wykresu, dobór liczby klastrów, 
# który wskazuje optymalne 'k'.
